...
class NuqueraCreateView(OperationalCreateView):
    ...
    def _present_param(self):
        ...
        asistencia, opcional). None significa que no se aplico ningun
        filtro (se muestra la cuadrilla completa); un set (incluso vacio)
        significa que se marco asistencia y hay que filtrar la lista de
        captura a esos IDs."""
    ...
    def _attendance_crew_param(self):
        """Cuadrilla para la que se esta marcando asistencia (paso
        independiente y opcional desde 'Nuevo trabajador de nuqueras'),
        separado de la cuadrilla elegida en 'Cuadrilla de trabajo'."""
        crew_id = self.request.GET.get("attendance")
        if not crew_id:
            return None
        try:
            return Crew.objects.get(pk=crew_id, active=True)
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        attendance_crew = self._attendance_crew_param()
        if attendance_crew is not None:
            # Paso opcional de asistencia, independiente de "Cuadrilla de
            # trabajo": vive junto a "Nuevo trabajador de nuqueras" para no
            # interponerse cuando solo se quiere ver/cambiar de cuadrilla.
            context["nuquera_attendance_mode"] = True
            context["nuquera_attendance_crew_id"] = attendance_crew.pk
            context["nuquera_attendance_crew_name"] = attendance_crew.name
            context["nuquera_attendance_workers"] = list(
                _nuquera_crew_worker_queryset(self.production, attendance_crew).values_list(
                    "pk", "full_name"
                )
            )
        crew = self._crew_param()
        if crew is not None:
            ...
            context["nuquera_crew_workers"] = (
                all_workers if present_ids is None else [w for w in all_workers if w[0] in present_ids]
            )
            quick_workers = _nuquera_quick_workers(self.production, crew)
            if present_ids is not None:
                quick_workers = [worker for worker in quick_workers if worker["pk"] in present_ids]
